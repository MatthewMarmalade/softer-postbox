...
curl -s -o "temporary_comic_page.html" "https://www.asofterworld.com/index.php?id=$1"

# extract the image link from the html
comic_file_name=$(grep -o 'https://www.asofterworld.com/clean/.*\.jpg' temporary_comic_page.html)

if [ $? -ne 0 ];
then
	echo "Failed to fetch comic $1, perhaps out of bounds?" >> error_log.txt
	echo "Failed to fetch comic $1, perhaps out of bounds?"
	rm temporary_comic_page.html
	exit 2
fi
...
curl -s -o "comic_$1.jpg" "$comic_file_name"
